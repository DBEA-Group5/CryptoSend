  ...
  const fetchWalletBalance = async (userId) => {
    try {
      const response = await axios.get(
        `https://personal-cq8qhlkg.outsystemscloud.com/WalletAPI/rest/WalletService/getAllWalletBalance?CustomerID`,
        {
          params: {
            CustomerID: userId,
          },
    ...
    <div className="flex flex-col min-h-screen bg-gray-900 p-6 max-w-md mx-auto">
          ...
          {[
            ...
            >
              <CardContent className="p-4 flex justify-between items-center">
                <div className="flex items-center">
                  <transaction.icon
                    className={`w-6 h-6 mr-3 ${transaction.color}`}
                  />
                  <div>
      ...
      <Card className="bg-gray-800 text-white mb-8 hover:bg-gray-700 transition-colors">
        <CardContent className="p-4 flex items-start">
          ...
          <p className="text-sm">
            Your balance includes all your accounts and investments. Tap on any
            item to see more details or to make changes.
          </p>
        </CardContent>
      </Card>
      ...
      <div className="grid grid-cols-2 gap-4 mt-auto">
        <Button
          variant="outline"
          className="text-white border-purple-500 hover:bg-purple-500 transition-colors"
        >
